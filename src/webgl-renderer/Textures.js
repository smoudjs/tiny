...
import { floorPow2, isPow2 } from '../utils/index.js';
...
function WebGLTextures( _gl, extensions, state, properties, capabilities, utils, info, textureGC ) {
	...
	function resizeImage( image, needsPowerOfTwo, needsNewCanvas, maxSize ) {
		...
		if ( scale < 1 || needsPowerOfTwo === true ) {
			...
			if ( ( typeof HTMLImageElement !== 'undefined' && image instanceof HTMLImageElement ) ||
				...
				const floor = needsPowerOfTwo ? floorPow2 : Math.floor;
	...
	function deallocateTexture( texture ) {
		...
		const i = managedTextures.indexOf(texture);
	...
	function uploadTexture( textureProperties, texture, slot ) {
		...
		const needsPowerOfTwo = textureNeedsPowerOfTwo( texture ) && isPowerOf2( texture.image ) === false;
		const image = resizeImage( texture.image, needsPowerOfTwo, false, maxTextureSize );

		const supportsMips = isPowerOf2( image ) || isWebGL2,
	...
	function setupRenderTarget( renderTarget ) {
		...
		const supportsMips = isPowerOf2( renderTarget ) || isWebGL2;
