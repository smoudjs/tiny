...
;// ./packages/gltf-loader/GLTFLoader.js
...
GLTFParser.prototype.parse = function (onLoad, onError) {
  ...
  this.markDefs();
  var dependencies = [this.getDependencies('scene'), this.getDependencies('animation')];
  var result = {
...
function computeBounds(geometry, primitiveDef, parser) {
  ...
  if (attributes.POSITION !== undefined) {
    ...
    if (min !== undefined && max !== undefined) {
      box.set(new Tiny.Vector3(min[0], min[1], min[2]), new Tiny.Vector3(max[0], max[1], max[2]));
    } else {
...
GLTFParser.prototype.loadMesh = function (meshIndex) {
  ...
  for (var i = 0, il = geometries.length; i < il; i++) {
    ...
    if (primitive.mode === WEBGL_CONSTANTS.TRIANGLES || primitive.mode === WEBGL_CONSTANTS.TRIANGLE_STRIP || primitive.mode === WEBGL_CONSTANTS.TRIANGLE_FAN || primitive.mode === undefined) {
      ...
      mesh = meshDef.isSkinnedMesh === true ? new Tiny.SkinnedMesh(geometry, material) : new Tiny.Mesh(geometry, material);
...
;// ./packages/gltf-loader/extends.js
