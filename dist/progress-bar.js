...
;// ./packages/progress-bar/ProgressBar.js
var defaultOptions = {
  bgColor: '#ffffff',
  bgAlpha: 1,
  value: 1,
...
var ProgressBar = function ProgressBar(options) {
  ...
  graphics.drawRoundedRect(options.strokeWidth / 2, options.strokeWidth / 2, options.width, options.height, options.radius);
  graphics.endFill();
  if (options.strokeWidth) {
    graphics.lineStyle(options.strokeWidth, options.strokeColor, options.strokeAlpha);
    graphics.drawRoundedRect(0, 0, options.width + options.strokeWidth, options.height + options.strokeWidth, options.radius + options.strokeWidth / 2);
  }
...
ProgressBar.prototype.setValue = function (value) {
  ...
  if (this.animated) {
    ...
    this._tween = this.game.tweens.add(tmpObj).to({
      value: value
    }, this.duration).easing(this.easing).onUpdate(function () {
