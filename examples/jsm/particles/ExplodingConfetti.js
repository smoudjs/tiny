...
export default class ExplodingConfettiParticle extends Tiny.Particle {
    ...
    onEmit() {
        ...
        this.p1.set(
            this.p0.x - this.parent.width + this.parent.width * 2 * Math.random(),
            this.p0.y - this.parent.height / 2 + this.parent.height * Math.random()
        );
        this.p2.set(
            this.p0.x - this.parent.width + this.parent.width * 2 * Math.random(),
            this.p0.y - this.parent.height / 2 + this.parent.height * Math.random()
        ...
        this.p3.set(
            this.p0.x - this.parent.width + this.parent.width * 2 * Math.random(),
            this.p0.y - this.parent.height / 2 + this.parent.height * 2
        );
    }
